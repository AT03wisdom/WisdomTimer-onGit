...
class AppDelegate: UIResponder, UIApplicationDelegate, UNUserNotificationCenterDelegate {
    ...
    func applicationWillResignActive(_ application: UIApplication) {
        ...
        database.set(Date(), forKey: "closedTime")
    }
    ...
    func applicationDidBecomeActive(_ application: UIApplication) {
        ...
        if let closedTime = database.object(forKey: "closedTime") {
            // アプリ中断時と再開時の時間の差分を求める
            let openedTime = Date()
            database.set(openedTime, forKey: "openedTime")
            let intervalTime = openedTime.timeIntervalSince(closedTime as! Date)
            // print("intervalTime : \(intervalTime)")
            database.set(intervalTime, forKey: "intervalTime")
        }
    }
